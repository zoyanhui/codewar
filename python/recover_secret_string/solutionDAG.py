def recoverSecret(triplets):
  ...
  DAG = createDAG(triplets)
  sortedeles = searchDAG(DAG)
  return ''.join(sortedeles)



def createDAG(triplets, reverse=True):
    DAG = dict()
    for triplet in triplets:
        for idx, ele in enumerate(triplet):
            setele = DAG.get(ele, None)
            if setele is None:
                setele = set()
            if reverse:
                for i in range(idx-1, -1, -1):
                    setele.add(triplet[i])
            else:
                for i in range(idx+1, len(triplet)):
                    setele.add(triplet[i])
            DAG[ele] = setele
    return DAG
